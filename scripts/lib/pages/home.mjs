...
function homeBody(ctx, page, H) {
  ...
  const paths = [
    { key: 'home', title: 'Homes', text: service('christmas-light-installation').summary, photo: 'roofline-large-home', property: 'Home', cta: 'Get a home estimate', link: ['See residential lighting', '/christmas-light-installation/'] },
    { key: 'hoa', title: 'HOAs and entrances', text: vertical('hoa-and-subdivision-entrances').summary, photo: 'park-tree-and-wraps', property: 'HOA or subdivision', cta: 'Get an entrance estimate', link: ['See HOA lighting', '/commercial/hoa-and-subdivision-entrances/'] },
    { key: 'downtown', title: 'Downtowns and municipalities', text: vertical('downtowns-and-municipalities').summary, photo: 'downtown-storefront-wraps', property: 'Downtown or municipality', cta: 'Plan a downtown estimate', link: ['See downtown lighting', '/commercial/downtowns-and-municipalities/'] },
    { key: 'business', title: 'Businesses', text: service('commercial-holiday-lighting').summary, photo: 'commercial-building-lit-trees', property: 'Business', cta: 'Book a walkthrough', link: ['See commercial lighting', '/commercial-holiday-lighting/'] },
  ];
  const pathCards = paths
    .map(
      (p, i) => `<li class="path" data-reveal style="--d:${i * 80}ms">
      <figure class="path-media">${photo(p.photo, { sizes: '(min-width: 1100px) 300px, (min-width: 700px) 50vw, 100vw' })}<figcaption>${esc(caption(p.photo))}</figcaption></figure>
      <div class="path-body">
        <h3>${esc(p.title)}</h3>
        <p>${esc(p.text)}</p>
        <div class="path-actions">
          <a class="btn btn-glow btn-sm" href="#estimate" data-cta="estimate" data-placement="path_${p.key}" data-preset-property="${esc(p.property)}"><span>${esc(p.cta)}</span>${icon('arrow')}</a>
  ...
  const cats = [
    { id: 'roof', label: 'Rooflines', layout: 'single', panes: ['roofline-large-home'], mpanes: ['roofline-large-home'], title: 'Rooflines', text: roof.hero_sub, list: roof.scope.slice(0, 4), link: ['Roofline Christmas lights', '/roofline-christmas-lights/'], add: 'Add roofline to my estimate', presetLight: 'Roofline' },
    ...
    { id: 'entrances', label: 'Entrances', layout: 'single', panes: ['subdivision-entrance-green-trees'], mpanes: ['subdivision-entrance-green-trees'], title: 'HOA and subdivision entrances', text: hoa.summary, list: hoa.scope.slice(0, 4), link: ['Entrance lighting', '/commercial/hoa-and-subdivision-entrances/'], add: 'Add an entrance to my estimate', presetLight: 'Entrance or sign', presetProperty: 'HOA or subdivision' },
...
<section class="sec sec-crew" aria-labelledby="crew-title">
  <div class="wrap crew">
    ...
    ${figure('bucket-truck-roofline-install', { sizes: '(min-width: 1000px) 460px, 100vw', cls: 'cfig cfig-b' })}
    ${figure('commercial-building-lit-trees', { sizes: '(min-width: 1000px) 760px, 100vw', cls: 'cfig cfig-c' })}
  </div>
</section>
